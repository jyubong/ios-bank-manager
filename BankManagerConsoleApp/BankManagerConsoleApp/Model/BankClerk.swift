...
struct BankClerk: CustomerReceivable {
    ...
    init(charge: Banking, startWork: @escaping (String) -> Void = { print($0) }, endWork: @escaping (String) -> Void = { print($0) }) {
        self.charge = charge
    ...
    func receive(customer: Customer) {
        ...
        let banking = charge.rawValue
        
        startWork("\(number)번 고객 \(banking)업무 시작")
        Thread.sleep(forTimeInterval: pace)
        endWork("\(number)번 고객 \(banking)업무 종료")
    }
}
